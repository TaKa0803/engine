...
void InstancingPSO::PreDraw(ID3D12GraphicsCommandList* commandList,const FillMode&fillMode,const BlendMode&blendMode) {
	//RootSignatureを設定。PSOに設定しているけど別途設定が必要
	commandList->SetGraphicsRootSignature(rootSignature.Get());
	commandList->SetPipelineState(graphicsPipelineState[static_cast<int>(fillMode)][static_cast<int>(blendMode)]);
}
